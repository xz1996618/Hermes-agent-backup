...
§
Hermes 工作目录：E:\13_WorkSpaceForHermes\xu_HermesAgent。之后工作产生的文件尽量保存到此目录，非必要不存C盘以节省C盘空间。用户默认工作目录为 E:\13_WorkSpaceForHermes。
§
板式换热器知识库路径：E:\13_WorkSpaceForHermes\01_bphe_knowledge_base\。回答板换（BPHE）相关问题时，优先检索此知识库。