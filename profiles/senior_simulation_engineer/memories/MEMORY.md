...
§
Hermes 离线安装器踩坑（详见 offline-hermes-installer-win skill）：Win cmd 用 GBK→.bat 须纯ASCII无BOM；系统命令用全路径；桌面快捷方式用 cscript+VBScript；C# 自解压桩从1MB偏移扫描标记。
§
...
§
创建 Skill 前必须先向用户提案（说明要创建什么Skill、用途、内容概要），得到明确批准后才能创建。不得自作主张直接创建Skill。
§
工作目录：E:\13_WorkSpaceForHermes\。仿真文件存 senior_simulation_engineer_HermesAgent\，板换知识库在 01_bphe_knowledge_base\，回答板换问题优先查知识库。非必要不写C盘。