...
§
对于从零搭建的新工具/项目，偏好先评估可行性再批准动工。常规任务直接执行。系统变更（删文件/禁服务/改设置）必须列清单+说明用途再同意。CLI优先于GUI，临时调用优先于常驻。
§
成本敏感，偏好免费模型。主模型 DeepSeek，视觉用 qwen3-vl-plus（DashScope/alibaba）。智谱 glm-4.7/4.7-flash/4v-flash/4.6v-flash 已确认免费，备用。sk-ws Key 只能识图不能生图。FAL_KEY 余额耗尽。微信已通过 iLink Bot 接 gateway。
§
